...
def compute_knn(backbone, data_loader_train, data_loader_val):
    ...
    for name, data_loader in data_loaders.items():
        for imgs, y in tqdm(data_loader, desc=f"Computing {name} embeddings for kNN"):
            imgs = imgs.to(device)
            lists[f"X_{name}"].append(backbone(imgs).cpu().numpy())
            lists[f"y_{name}"].append(y.cpu().numpy())
...
def compute_embedding(
    ...
    for img, y in tqdm(data_loader, desc="Computing embeddings"):
        img = img.to(device)
